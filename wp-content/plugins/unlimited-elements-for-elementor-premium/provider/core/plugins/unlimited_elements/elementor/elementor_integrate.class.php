...
class UniteCreatorElementorIntegrate{
    ...
    private function preloadElementorDBData(){
    	...
    	if(self::$isOutputPage){
    		...
    		$query .= " inner join $tableCats as cats on(cats.id = addons.catid)"; 
	...
	public function onFrontAfterRender($objElement){
		...
		$backgroundType = UniteFunctionsUC::getVal($settings, "uc_background_type");
	...
	public function onPrintFooterHtml(){
		...
		dmp("print test widgets footer");
		dmp($this->arrBGAddonsOutput);
		exit();
