	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		if ( 'ltr' === $list_direction ) {
			...
			if ( '5px' !== $text_indent ) {
				...
				et_pb_responsive_options()->generate_responsive_css( $text_indent_values, $text_selector, 'padding-right', $render_slug );
			}
		...
		$list_alignment_phone_style  = '' !== $list_alignment_phone ? sprintf( 'justify-content: %1$s;', esc_attr( $list_alignment_phone ) ) : '';
		...
		if ( '' !== $list_space_between ) {
			...
			et_pb_responsive_options()->generate_responsive_css( $list_space_between_values, '%%order_class%% .dsm_icon_list_items .dsm_icon_list_child:not(:last-child)', 'margin-bottom', $render_slug );
		...
		$output = sprintf(
			'<ul class="dsm_icon_list_items dsm_icon_list_%2$s_direction">%1$s</ul>',
			$this->content,
