	...
	class DiviMegaPro {
		...
		protected static function init_hooks() {
			...
			if ( !is_admin() && !self::is_divi_builder_enabled() && $pos === false ) {
				...
				add_action( 'wp_head', array( 'DiviMegaPro', 'getAllDiviMegaPros'), 100 );
				
				// Check for possible required dependencies
				add_action( 'wp_head', array( 'DiviMegaPro', 'checkAllDiviMegaPros') );
				
				// Divi Mega Pro call
				add_action( 'wp_footer', array( 'DiviMegaPro_Controller', '_init' ), 10 );
				add_action( 'wp_footer', array( 'DiviMegaPro_Controller', 'showDiviMegaPro' ), 1, 1 );
			}
		}
		...
		protected static function load_wc_scripts() {
			...
			if ( ! class_exists( 'WC_Frontend_Scripts' ) && function_exists( 'et_fb_enabled' ) && ! et_fb_enabled() ) {
				return;
			}
				...
				if ( function_exists( 'et_theme_builder_frontend_render_layout' ) ) {
					...
					if ( is_array( $layouts ) && array_filter( $layouts ) ) {
						...
						foreach( $layouts as $layout_type => $layout_ ) {
							...
							if ( isset( $layout_['id'] ) && $layout_['enabled'] === true ) {
