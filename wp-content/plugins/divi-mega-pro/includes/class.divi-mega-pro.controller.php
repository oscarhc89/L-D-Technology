	...
	class DiviMegaPro_Controller extends DiviMegaPro {
		...
		public static function _init() {
			...
			if ( et_get_option( 'et_pb_css_in_footer', 'off' ) === 'off' && $divi_styles ) {
				...
				self::$post_id = $is_singular ? et_core_page_resource_get_the_ID() : 'global';
				
				foreach( $divi_styles as $divi_style ) {
				
					$files = glob( $divi_style->BASE_DIR . '/et-' . $divi_style->owner . '-*' );
					...
					if ( $files ) {
						...
						foreach( (array) $files as $file ) {
							...
							if ( ! self::$data_utils->starts_with( $file, $divi_style->BASE_DIR ) ) {
					...
					else if ( isset( $divi_style->data ) ) {
						
						ksort( $divi_style->data, SORT_NUMERIC );
						
						$resource_data = $divi_style->data;
						
						foreach ( $resource_data as $priority => $data_part ) {
							foreach ( $data_part as $data ) {
								$custom_divi_css .= $data;
							}
						}
					}
				}
		...
		public static function showDiviMegaPro( $render = true ) {
			...
			try {
				...
				divimegapro_singleton['header'] = <?php print $header ?>;
				divimegapro_singleton['content'] = <?php print $content ?>;
				divimegapro_singleton['footer'] = <?php print $footer ?>;
